        ...
        <div className="absolute top-[110%] left-0 z-50 bg-white rounded-lg shadow-xl p-4 w-72 select-none border border-gray-200">
            <div className="flex items-center justify-between mb-4">
                <button type="button" onClick={() => setViewDate(new Date(year, month - 1, 1))}
                    className="text-2xl text-gray-500 hover:text-gray-800 px-2 leading-none bg-transparent border-none cursor-pointer">‹</button>
            ...
            <div className="grid grid-cols-7 gap-0.5">
                {cells.map((cell, i) => {
                    ...
                    return (
                        <div key={i} onClick={() => handleSelect(cell.day, cell.current)}
                            className={["text-center py-1.5 rounded-full text-sm cursor-pointer transition-colors",
...
function FloatingInput({ label, value, onChange, type = "text", error }) {
    ...
    return (
        <div>
            <div className={`relative h-11 pl-1 rounded-md border bg-white transition-colors
                ${focused ? "border-blue-600 shadow-[0_0_0_5px_rgba(38,132,255,.2)]" : "border-gray-300"}`}>
                <label className={`absolute left-4 font-semibold tracking-tight uppercase pointer-events-none transition-all duration-150
    ...
    return (
        <div ref={ref} className="relative">
            <div onClick={() => setOpen(!open)}
                ...
                <label className={`absolute left-3.5 top-1.5 text-[10.24px] font-semibold tracking-tight uppercase pointer-events-none
                    ${open ? "text-blue-600" : "text-gray-400"}`}>{label}</label>
                <span className={`text-sm ${selectedLabel ? "text-gray-800" : "text-gray-400"}`}>{selectedLabel}</span>
                <ChevronDown size={16} className={`absolute right-3.5 top-1/2 -translate-y-1/2 text-gray-400 transition-transform ${open ? "rotate-180" : ""}`} />
            </div>
            {open && (
    ...
    return (
        <div ref={ref} className="relative">
            <div onClick={() => setOpen(!open)}
                ...
                <div className={`text-[10.24px] font-semibold tracking-tight uppercase ${open ? "text-blue-600" : "text-gray-400"}`}>{label}</div>
...
function MultiSelect({ label, selected, onChange, options, placeholder }) {
    ...
    return (
        <div ref={ref} className="relative">
                ...
                <div className="flex flex-wrap gap-1 pr-6">
                    {selected.length === 0 ? (
                        ...
                        selected.map(val => {
                            ...
                            return item && (
                                <span key={val} className="inline-flex items-center gap-1 px-2 mt-1 rounded bg-blue-50 border border-blue-200 text-xs text-blue-700">
                                    {item.label}
                                    <span onClick={e => { e.stopPropagation(); onChange(selected.filter(v => v !== val)); }}
                                        className="cursor-pointer text-gray-400 hover:text-gray-700">×</span>
                                </span>
                            );
                        })
                    )}
            ...
            {open && (
                <div className="absolute top-[105%] left-0 right-0 bg-white border border-gray-200 rounded-md z-50 shadow-lg overflow-hidden">
                    {options.filter(opt => !selected.includes(opt.value)).map((opt, i) => (
                        <div key={i} onClick={() => toggle(opt.value)}
                            className="px-4 py-2.5 text-sm cursor-pointer text-gray-800 hover:bg-gray-50 transition-colors">
                            {opt.label}
                        </div>
                    ))}
                </div>
        ...
        <div ref={ref} className="relative">
            <div onClick={() => setOpen(!open)}
                className={`border rounded-md bg-white px-3.5 pt-2 pb-2 cursor-pointer min-h-[46px] flex flex-wrap gap-2 items-center transition-colors
                    ${open ? "border-blue-600 shadow-[0_0_0_5px_rgba(38,132,255,.2)]" : "border-gray-300"}`}>
                <div className={`text-[10.24px] font-semibold tracking-tight uppercase w-full ${open ? "text-blue-600" : "text-gray-400"}`}>ADD TAGS</div>
                {selected.map(id => {
                    ...
                    return tag ? (
                        <span key={id} className="inline-flex items-center gap-1 bg-blue-50 border border-blue-200 rounded px-2 py-0.5 text-[11px] font-bold text-blue-700 uppercase tracking-wider">
                            {tag.name}
                            <span onClick={e => { e.stopPropagation(); onChange(selected.filter(v => v !== id)); }}
                                className="text-gray-400 hover:text-gray-700 text-sm leading-none cursor-pointer">×</span>
                        </span>
                    ) : null;
                })}
            </div>
            {open && (
                <div className="absolute top-[105%] left-0 right-0 bg-gray-50 border border-gray-200 rounded-md z-50 shadow-lg overflow-hidden">
                    {allTags.filter(tag => !selected.includes(tag.id)).map(tag => (
...
function FloatingTextarea({ label, value, onChange }) {
    ...
    return (
        <div className={`relative border rounded-md bg-white px-3 pt-7 pb-2 my-7 transition-colors ${focused ? "border-blue-600 shadow-[0_0_0_5px_rgba(38,132,255,.2)]" : "border-gray-300"}`}>
            <label className={`absolute left-3.5 transition-all duration-150 font-semibold tracking-tight uppercase pointer-events-none
                ${focused || value ? "top-1 text-[10.24px]" : "top-2 text-[11px]"}
                ${focused ? "text-blue-600" : "text-gray-400"}`}>{label}</label>
...
// ─── Main Edit Page ───────────────────────────────────────────────────────────
export default function Edit({ project, teams, clients, tags, users, admins, taskViews, privacyOptions, errors }) {
    const { data, setData, put, processing } = useForm({
        name:               project?.name              ?? "",
        default_task_view:  project?.default_task_view ?? "",
    ...
    // Option arrays — identical pattern to Create
    ...
    return (
        <DashboardLayout>
            <div className="m-10">
                ...
                {/* Title */}
                <h1 className="text-3xl font-black tracking-wider text-gray-900 mb-6 [paint-order:stroke_fill] [-webkit-text-stroke:0.75px_#111827]">
                    Edit Project
                </h1>
